        ...
        <article class="card pull-right">
            <div id="safaris">
                ...
                <table class="table">
                    ...
                    @foreach ($logbookCarSearches as $logbook)
                    <tbody>
                        <tr>
                            <td scope="row">
                                {{ ucwords($logbook->name) }}
                            </td>
                            
                            <td>
                                {{ ucwords($logbook->phone) }}
                            </td>
                            
                            <td>
                                {{ ucwords($logbook->registration_no) }}
                            </td>

                            <td>
                                <div class="manage-safari pull-right">
                                    <a href="{{ route('vehicle-transfer-edit', $logbook) }}">
                                        <i class="lni-pencil-alt"></i>
                                        Edit
                                    </a>

                                    <a onclick="confirm('Are you sure to delete this request?') ? document.getElementById('delete-form-{{ $logbook->id }}').submit() : NaN">
                                        <i class="lni-pencil-alt"></i>
                                        Delete
                                    </a>

                                    <form id="delete-form-{{ $logbook->id }}" action="{{ route('delete-user', $logbook) }}" method="post">
