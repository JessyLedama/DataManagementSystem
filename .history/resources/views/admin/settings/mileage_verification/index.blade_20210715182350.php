        ...
        <article class="card pull-right">
            <div id="safaris">
                ...
                <table class="table">
                    ...
                    @foreach ($mileageVerifications as $mileageVerificati)
                    <tbody>
                        <tr>
                            <td scope="row">
                                {{ ucwords($mileageVerificati->name) }}
                            </td>
                            
                            <td>
                                {{ ucwords($mileageVerificati->phone) }}
                            </td>
                            
                            <td>
                                {{ ucwords($mileageVerificati->email) }}
                            </td>

                            <td>
                                {{ ucwords($mileageVerificati->car_chasis) }}
                            </td>

                            <td>
                                <div class="manage-safari pull-right">
                                    <a href="{{ route('vehicle-transfer-edit', $mileageVerificati) }}">
                                        <i class="lni-pencil-alt"></i>
                                        Edit
                                    </a>

                                    <a onclick="confirm('Are you sure to delete this request?') ? document.getElementById('delete-form-{{ $mileageVerificati->id }}').submit() : NaN">
                                        <i class="lni-pencil-alt"></i>
                                        Delete
                                    </a>

                                    <form id="delete-form-{{ $mileageVerificati->id }}" action="{{ route('delete-user', $mileageVerificati) }}" method="post">
